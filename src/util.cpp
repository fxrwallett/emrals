...
const char * const BITCOIN_CONF_FILENAME = "emrals.conf";
const char * const BITCOIN_PID_FILENAME = "emralsd.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "emrals" is a composite category enabling all Emrals-related debug output
            if(ptrCategory->count(string("emrals"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\EmralsCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\EmralsCore
    // Mac: ~/Library/Application Support/EmralsCore
    // Unix: ~/.emralscore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "EmralsCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/EmralsCore";
#else
    // Unix
    return pathRet / ".emralscore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty emrals.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override emrals.conf
